...
board_info() {
          ...
          manufacturer=$(grep -m1 -oP "(?<=^ro.product.brand=).*" -hs {odm}/build*.prop | head -1)
          ...
          brand=$(grep -m1 -oP "(?<=^ro.product.brand=).*" -hs {odm,system,system/system,vendor}/build*.prop | head -1)
